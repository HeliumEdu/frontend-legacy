 ...
 * project and interested in joining forces? Reach out and let us know! contact@heliumedu.com
 *
 * @license MIT
 * @version 1.17.16
 */
...
function HeliumClasses() {
    ...
    this.create_course_for_group_btn = function () {
        ...
        let course_group = $("#course-group-tabs li.active a").attr("href") ? $("#course-group-tabs li.active a")
            .attr("href")
        ...
        $("#course-group").val(course_group);

        $("a[href='#course-panel-tab-1']").tab("show");

        $("#course-modal-label").html("Add Class");
        $("#course-title").val("");
        $("#course-start-date").datepicker("setDate", start_date.toDate());
        $("#course-end-date").datepicker("setDate", end_date.toDate());
     ...
     */
    this.show_hide_schedule_times = function (day, any_shown) {
        ...
        if (!self.edit) {
            ...
            helium.planner_api.get_course(function (data) {
                if (helium.data_has_err_msg(data)) {
                    ...
                    helium.ajax_calls.push(helium.planner_api.get_categories_by_course_id(function (data) {
        ...
        bootbox.dialog(
            {
                ...
                buttons: {
                    "delete": {
                        ...
                        "callback": function () {
                            $("#loading-courses").spin(helium.SMALL_LOADING_OPTS);
                            helium.ajax_calls.push(helium.planner_api.delete_course(function (data) {
                                if (helium.data_has_err_msg(data)) {
                                    helium.ajax_error_occurred = true;
                                    $("#loading-courses").spin(false);
                ...
                if (!helium.ajax_error_occurred) {
                    ...
                    modified_categories.sort(function (a, b) {
                        ...
                        const weight2 = $($(b).children()[1]).text() !== "N/A" ?
                                        parseFloat($($(b).children()[1]).text().slice(0, -1)) : 0;
                        if (weight1 < weight2) {
                            return -1;
                    ...
                    modified_categories.each(function () {
                        helium.planner_api.edit_category(function (data) {
                                                             if (helium.data_has_err_msg(data)) {
                                                                 helium.ajax_error_occurred = true;
                                                                 $("#loading-course-modal").spin(false);
                            ...
                            if (helium.data_has_err_msg(course)) {
                                ...
                                if (!helium.ajax_error_occurred) {
                                    $.each(categories_data, function (i, category_data) {
                                        ...
                                        }, self.course_group_id, self.edit_id, category_data, false);
                                    });
                                }
    ...
    this.refresh_course_groups = function () {
        ...
        $("table[id^='course-group-table-']").each(function () {
            ...
            self.course_groups[id] = {
                ...
                end_date: $("#course-group-" + id + "-end-date").html().trim()
            };
        });
    };
}
...
$(document).ready(function () {
    ...
    $(".weight-help").on("click", function () {
        window.open("https://landing.heliumedu.com/support/grades-and-progress/setting-up-weighted-grading-assignment-categories/");
    });
